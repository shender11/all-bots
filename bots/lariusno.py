...
TOKEN = "8766653531:AAGuaR_WVVESijvUQYUZz2rYn0SzQ4L92iU"
ADMIN_ID = 8718436363
OWNER_ID = 1826030998
SHEET_KEY = "1an9FadZtfk5gJshZRF1BwRjsw0E8hnaXfVbjXUeZKYo"
...
main_keyboard = ReplyKeyboardMarkup(
    keyboard=[
        [KeyboardButton(text="Start Shift")],
        [KeyboardButton(text="End Shift")],
        [KeyboardButton(text="Breaks")],
        [KeyboardButton(text="Days Off")],
...
days_keyboard = ReplyKeyboardMarkup(
    keyboard=[
        ...
        [KeyboardButton(text="Cancel Day Off")],
        ...
        for r in records:
            ...
            try:
                ...
                break_data[user_id] = {
                    ...
                    "active": True,
                    "name": r[1],
                    "username": r[2] if r[2] != "no username" else None
                }
            except:
                pass
    except:
...
def get_today_admin_stats():
    ...
    for r in dayoff_records:
        if len(r) > 3 and r[1] == today_str:
            dayoff_users.append(f"@{r[3]}" if r[3] != "no username" else f"ID: {r[2]}")
...
async def break_control(user_id, minutes, name, username):
    ...
    if minutes > 5:
        ...
        if user_id in break_data and user_id not in blocked_users:
            await bot.send_message(user_id, "⏳ 5 minutes left until the end of your break")
    ...
    while user_id in break_data and break_data[user_id]["active"] and user_id not in blocked_users:
        admin_text = (
            f"🚨 OVERDUE ON BREAK, CALL IMMEDIATELY!\n"
            f"{name}\n"
...
async def handle(message: Message):
    ...
    if message.text in [
        ...
        "Breaks", "Days Off", "Salary", "My Profile",
        "Back",
        "Start Break", "End Break",
        "15 minutes", "30 minutes",
        "Take Day Off", "Cancel Day Off",
        "My Days Off",
        "My Salary"
    ]:
        try:
    ...
    if message.text == "Start Shift":
        ...
        if user_id in shift_data:
            await send_clean_message(user_id, "❗ You already have an active shift", reply_markup=main_keyboard)
            return
        ...
        await send_clean_message(user_id, "✅ Shift started", reply_markup=main_keyboard)
        return

    if message.text == "End Shift":
        waiting_time.discard(user_id)
        salary_waiting.pop(user_id, None)

        if user_id not in shift_data:
            await send_clean_message(user_id, "❗ You do not have an active shift", reply_markup=main_keyboard)
            return
        ...
        await send_clean_message(user_id, "✅ Shift ended", reply_markup=main_keyboard)
        return

    if message.text == "Breaks":
        waiting_time.discard(user_id)
        salary_waiting.pop(user_id, None)
        await send_clean_message(user_id, "Break menu", reply_markup=break_keyboard)
        return

    elif message.text == "Days Off":
        waiting_time.discard(user_id)
        salary_waiting.pop(user_id, None)
        await send_clean_message(user_id, "Days off menu", reply_markup=days_keyboard)
        return

    elif message.text == "Salary":
        waiting_time.discard(user_id)
        salary_waiting.pop(user_id, None)
        await send_clean_message(user_id, "Salary menu", reply_markup=salary_keyboard)
        return
        ...
        text = (
            ...
            f"Days off left: {remaining_days_off}\n"
            f"Breaks today: {breaks_count}\n"
            f"15-minute breaks: {breaks_15}\n"
            f"30-minute breaks: {breaks_30}\n"
            f"Minutes on break today: {total_minutes}\n"
            f"Break minutes left: {remaining_break_minutes}"
        )
        ...
        waiting_time.discard(user_id)
        salary_waiting.pop(user_id, None)
        await send_clean_message(user_id, "Main menu", reply_markup=main_keyboard)
        return
    ...
    if message.text == "Start Break":
        ...
        if user_id in break_data and break_data[user_id]["active"]:
            await send_clean_message(user_id, "❗ You already have an active break", reply_markup=break_keyboard)
            return
        ...
        if user_id not in break_data:
            await send_clean_message(user_id, "No active break", reply_markup=break_keyboard)
            return
        ...
        await send_clean_message(user_id, "Break ended", reply_markup=break_keyboard)
        return
        ...
        await send_clean_message(user_id, "Choose a day off to cancel:", reply_markup=keyboard)
        return

    elif message.text == "My Salary":
        waiting_time.discard(user_id)
        salary_waiting[user_id] = {"step": "balance"}
        await send_clean_message(user_id, "Enter balance ($)", reply_markup=salary_keyboard)
        return
        ...
        if step == "balance":
            try:
                ...
                await send_clean_message(user_id, "Enter a number", reply_markup=salary_keyboard)
                return

            salary_waiting[user_id]["balance"] = balance
            salary_waiting[user_id]["step"] = "percent"
            await send_clean_message(user_id, "Enter percentage (for example 45)", reply_markup=salary_keyboard)
            return
            ...
            await send_clean_message(user_id, "Enter gifts amount ($)", reply_markup=salary_keyboard)
            return
            ...
            await send_clean_message(user_id, "Enter gifts percentage (20-25)", reply_markup=salary_keyboard)
            return
        ...
        await send_clean_message(user_id, f"Break started for {minutes} min", reply_markup=break_keyboard)

        text = (
            f"🟡 Started break ({minutes} min)\n"
            f"{message.from_user.full_name}"
        )
...
async def ignore_click(callback: CallbackQuery):
    await callback.answer("Unavailable", show_alert=False)
...
async def select_day(callback: CallbackQuery):
    ...
    if len(user_days) >= 6:
        await callback.message.answer("❌ You already have 6 days off this month")
        return
    ...
    if already_taken:
        await callback.message.answer("❌ You already took this day")
        return
    ...
    if len(same_day) >= get_team_limit():
        await callback.message.answer("❌ No spots left for this day")
        return
    ...
    text = (
        f"📅 Took a day off\n"
        f"@{callback.from_user.username if callback.from_user.username else 'no username'}\n"
        f"{selected_date.strftime('%d.%m.%Y')}\n"
        f"Spots left: {remaining}"
        f"{list_text}"
    )
...
async def cancel_day(callback: CallbackQuery):
    ...
    if date == today_str:
        await bot.send_message(
            user_id,
            "❌ You cannot cancel a day off on the same day",
            reply_markup=days_keyboard
        )
        return
...
async def today_stats(message: Message):
    ...
    if stats:
        ...
        for user_id, data in stats.items():
            username_text = f"@{data['username']}" if data["username"] != "no username" else f"ID: {user_id}"
            late_mark = " | Was late" if user_id in late_users else ""
            text += f"{data['name']} ({username_text}) — {data['count']} breaks, {data['minutes']} min{late_mark}\n"
    else:
...
async def block_user(message: Message):
    ...
    try:
        ...
        await message.answer(f"Blocked: {user_id}")
    except:
        await message.answer("Error. Example: /block 123456789")
...
async def unblock_user(message: Message):
    ...
    try:
        ...
        await message.answer(f"Unblocked: {user_id}")
    except:
        await message.answer("Error. Example: /unblock 123456789")
...
async def delete_user(message: Message):
    ...
    try:
        ...
        await message.answer("Error. Example: /delete 123456789")
